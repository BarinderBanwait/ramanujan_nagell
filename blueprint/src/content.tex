...
\begin{lemma}[Coprimality]
  ...
  \uses{def:R, lem:norm_eq, lem:theta_mul_theta', lem:theta_sq, def:theta, def:theta'}
  \leanok
...
\begin{lemma}[Must have minus sign]
  ...
  \uses{lem:theta'_eq_one_sub_theta, lem:theta'_irreducible, lem:theta_mul_theta', lem:units_pm_one, def:theta, def:theta'}
  \leanok
...
A single generic lemma handles all five cases: it takes an equation $x^2 = a^2$
(where $a^2 = 2^n - 7$) and the value of $n$, and identifies the solution pair $(x, n)$.

\begin{lemma}[Solving $x^2 = a^2$]
  \label{lem:solution_from_sq}
  \lean{solution_from_sq}
  \leanok
  If $x^2 = a^2$ and $n = m$, then $(x, n) = (a, m)$ or $(x, n) = (-a, m)$.
\end{lemma}
\begin{proof}\leanok
  From $x^2 = a^2$ we get $x = a$ or $x = -a$.
\end{proof}
...
\begin{theorem}[Ramanujan--Nagell]
  ...
  \uses{thm:odd_case_only_three_values, lem:ramanujan_nagell_even_pow_factors, lem:solution_from_sq}
  \leanok
...
\begin{proof}
  ...
  giving $n = 4$ and $x = \pm 3$ (Lemma~\ref{lem:solution_from_sq}).

  \textbf{Case 2: $n$ odd, $n = 3$.} Direct computation gives $x^2 = 1$, so $x = \pm 1$
  (Lemma~\ref{lem:solution_from_sq}).
